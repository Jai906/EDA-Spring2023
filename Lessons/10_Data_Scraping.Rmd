...
scrape.it <- function(the_year, the_facility){
  ...
  #Sys.sleep(1) #uncomment this if you are doing bulk scraping! #tells R to wait one second,runs code more slowly;5 seconds is better
...
#Use lapply to apply the scrape function; one variable is static= the facilicty
the_dfs <- lapply(X = the_years,
                  FUN = scrape.it,
                  the_facility=my_facility)

#OR, use purrr's map function; same as lapply
...
the_facility_ids <- the_main_website %>% 
  html_nodes('#content a') %>%    #Extract a list of html nodes tagged with "content a"
...
```{r}
#Create a subset of facilities; want three years for each site
