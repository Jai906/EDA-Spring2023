...
```{r project.setup, eval= TRUE, echo= TRUE, message= FALSE, warning= FALSE}
#Import libraries
library(tidyverse);library(lubridate);library(here);library(knitr);library(viridis)
...
```{r ozone boxplot 2018, echo=FALSE, warning=FALSE, fig.align='left', fig.cap="Ozone concentrations by for 2018", fig.height=3}
epa_data %>% 
  filter(Year == 2018) %>% 
  ggplot(aes(x = as.factor(Month), y = Ozone, fill = Month)) +
  xlab("Month") +
  ylab("Ozone (µg/m3)") +
...
```{r ozone boxplot 2019, echo=FALSE, warning=FALSE, fig.align='left', fig.cap="Ozone concentrations by for 2019", fig.height=3}
epa_data %>% 
  filter(Year == 2019) %>% 
  ggplot(aes(x = as.factor(Month), y = Ozone, fill = Month)) +
  xlab("Month") +
...
Link to repository -> [Git](https://github.com/Jai906/EDA-Spring2023)
